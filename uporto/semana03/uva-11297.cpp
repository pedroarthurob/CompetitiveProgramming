...
Node merge(Node leftNode, Node rightNode) {
    ...
    answer.maxValue = max(leftNode.maxValue, rightNode.maxValue);
    answer.minValue = min(leftNode.minValue, rightNode.minValue);
    return answer;
}
...
int main() {
    ...
    for(int i=0; i < q; i++) {
        ...
        if (op == 'q') {
            int x1, y1, x2, y2;
            cin >> x1 >> y1 >> x2 >> y2;
            Node answer = query_x(1, 1, n, x1, y1, x2, y2);
            cout << answer.maxValue << " " << answer.minValue << endl;
        } else {
            int x, y, v;
            cin >> x >> y >> v;
